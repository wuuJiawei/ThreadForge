...
[![Changelog](https://img.shields.io/badge/Changelog-v1.2.3-0ea5e9)](./CHANGELOG.md)
...
- 修复任务 runner 中断状态在线程池复用时泄漏，避免 JDK 8 后续 Channel/deadline 任务提前取消。
- 新增确定性线程复用回归测试。

### 1.2.2 变更摘要

- 修复任务生命周期、超时取消、Scope 关闭和任务注册竞态。
- 隔离内部控制调度器，修复 Channel 中断、FAIL_FAST 完成顺序和上下文恢复问题。
- 增加 integrations、examples、benchmarks 以及并发竞态验证。
...
- 高阶编排 API：`JoinStrategy` + `ScopeJoiner`（`firstSuccess` / `quorum` / `hedged`）
...
- If you are running examples or benchmarks against an unpublished local change, install the local artifact first:
...
- release tag 格式固定为 `vX.Y.Z`，例如 `v1.2.3`
